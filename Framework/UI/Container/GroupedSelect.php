...
use BDB\Framework\UI\Component\Label;
...
class GroupedSelect extends AContainer {
    ...
    public function __construct($name, $id, array $valueSets, $questionHeading = '') {
        ...
        foreach($valueSets as $set) {
            ...
            $label = new \BDB\Framework\UI\Component\Label("{$name}_{$current}_label", "{$id}_{$current}_label", $set['DisplayText']);
            $label->SetFor("{$id}_{$current}");
